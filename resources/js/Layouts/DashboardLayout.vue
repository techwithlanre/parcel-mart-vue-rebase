...
<template>
  ...
  <div class="min-h-screen flex flex-col bg-white">
    ...
    <div :class="{'w-64': mobileSidebarShow === true}" style="z-index: 100" class="fixed flex flex-col top-14 left-0 w-0 md:w-64 bg-primary h-full text-white transition-all duration-500 border-none sidebar">
      <div class="overflow-y-auto overflow-x-hidden flex flex-col justify-between flex-grow">
        <ul class="flex flex-col py-4 space-y-1" v-for="group  in sidebar">
          <li v-for="item in group" class="pt-3">
          ...
          <li v-if="$page.props.auth.user.is_admin === 1" v-for="item in adminMenu" :key="item" class="mt-10 pt-3">
    ...
    <div class="h-full ml-0 mt-14 mb-10 md:ml-64 p-5 md:p-10" style="z-index: 0.5">
      <slot />
    </div>
  </div>
</template>
