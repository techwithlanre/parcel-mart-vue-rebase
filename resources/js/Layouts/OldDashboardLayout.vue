    ...
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white sdark:bg-gray-800">
      <ul class="space-y-2 font-medium" v-for="group  in sidebar">
        <li v-if="page.props.auth.user.is_admin === 0" v-for="item in group" class="">
          <Link :href="item.route" class="flex items-center hover:text-primary p-2 text-gray-900 rounded-lg hover:bg-gray-100 group" :class="{'text-white bg-primary group font-bold rounded-md py-2 hover:text-primary hover:bg-background': $page.url.startsWith(item.route)}">
