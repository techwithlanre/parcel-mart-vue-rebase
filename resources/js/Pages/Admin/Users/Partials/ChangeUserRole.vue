...
const submit = () => {
  form.put(route('users.change-user-role', props.user.id), {
    onFinish: () => {
...
<template>
  <div>
    <div class="flex flex-row justify-between items-start p-5">
      <div class="">
        <h3>Chane Admin User Role</h3>
        <p class="text-sm">Use this form to change admin user role</p>
      </div>
      <div>
      ...
      <form action="" @submit.prevent="submit">
        <div class="flex flex-col gap-y-4">
          <div class="flex lg:flex-row flex-col w-full gap-x-10 gap-y-4">
            <div class="w-full">
              <InputLabel value="Select user role" class="font-normal"/>
              <SelectInput required type="number" v-model="form.role" :options="roles" min="1"
                         placeholder="" class="mt-2"/>
            </div>
          </div>
          <div class="flex lg:flex-row flex-col items-center w-full gap-x-10 gap-y-4">
            <PrimaryButton :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Change Role</PrimaryButton>
          </div>
        </div>
      </form>
    </div>
