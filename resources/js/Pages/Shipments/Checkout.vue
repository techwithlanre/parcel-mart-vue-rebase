    ...
    <DashboardLayout page-title="Shipment Checkout">
        ...
        <div class="flex sm:flex-row-reverse flex-col-reverse justify-between gap-10 mt-10">
            <form @submit.prevent="pay"
                  class="card sm:p-10 p-5 bg-white rounded-xl shadow hover:shadow-lg duration-500 sm:w-1/2 w-full">
                <div>
                        ...
                        <div v-for="item in shipping_rate_log" class="card shadow-sm border border-gray-100 rounded-xl">
                            ...
                            <label class="cursor-pointer">
                                <div class="p-4 font-bold flex flex-row justify-between bg-gray-50 rounded-t-xl">
                                    ...
                                    <input type="radio"
                                           class="text-primary border-primary focus:ring-opacity-30 focus:ring-primary"
                                           :value="item" v-model="form.option" v-on:change="setOption"/>
                                </div>
                                <hr class="border-gray-100">
                                <div class="p-4">
                                    <p class="text-sm">Amount: {{
                                            naira.format(parseFloat(item.charge_before_tax))
                                        }}</p>
                    ...
                    <div v-if="form.option_id > 0" class="transition-all duration-500">
                        <div class="mt-5 flex sm:flex-row flex-col justify-between gap-x-10">
                            <div class="w-full">
                                ...
                                <TextInput type="datetime-local" v-model="form.shipment_date" min="" max=""
                                           class="mt-3"/>
                                <!--                                <VueDatePicker
                    ...
                    <div
                        class="mt-10 p-5 shadow-sm border border-gray-100 rounded-xl duration-500 transition-all flex flex-row justify-between items-center"
                        v-if="form.insurance.length > 0">
                        <div>
                            <div class="text-lg font-bold">{{ insurance_options[form.insurance - 1].name }}</div>
                            <div>Covers damages upto {{
                                    naira.format(insurance_options[form.insurance - 1].cover)
                                }}
                            </div>
                        </div>
                        <div class="text-green-600 bg-background px-4 py-1 rounded-full">
                            {{ naira.format(insurance_options[form.insurance - 1].amount) }}
                        </div>
                    </div>
                    ...
                    <div class="mt-10">
                        ...
                        <p class="text-sm">Shipping Amount:
                            <span v-if="form.option">{{
                                    naira.format(form.option.total_charge.replace(',', ''))
                                }}</span>
                            <span v-else>0.00</span>
                        </p>
                        <p class="text-sm">Insurance: <span v-if="form.insurance.length > 0">{{
                                naira.format(insurance_options[form.insurance - 1].amount)
                            }}</span><span v-else>0.00</span></p>
                    </div>
                    ...
                    <div class="mt-10">
                        <PrimaryButton type="submit" class="duration-500"
                                       :class="{ 'opacity-25': form.processing || !confirm }"
                                       :disabled="!confirm || form.processing">
                            Book & Pay
                            <span v-if="form.option">{{
                                    naira.format(parseFloat(form.option?.total_charge) + parseFloat(insurance_options[form.insurance - 1]?.amount))
                                }}</span>
                            <span v-else>0.00</span>
                        </PrimaryButton>
                                ...
                                <div class="flex flex-row justify-between gap-x-10 rounded-xl ml-5 mt-5">
                                    <div class="card bg-white duration-500 w-full">
                                        ...
                                        <div class="flex gap-x-10">
                                            <div class="text-primary font-bold text-sm">{{ origin_location.city }},
                                                {{ origin_location.state }}, {{ origin_location.country }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="mb-3 mt-3 flex flex-row justify-end h-max">
                                        <Link :href="route('shipment.origin', shipment.id)">
                                            <button
                                                class="shadow p-2 rounded-full bg-background/30 flex gap-x-3 flex-row justify-between items-center">
                            ...
                            <div class="mt-10">
                                ...
                                <div class="flex flex-row justify-between gap-x-10 rounded-xl ml-5 mt-5">
                                    <div class="card bg-white duration-500 w-full">
                                        ...
                                        <div class="flex gap-x-10">
                                            <div class="text-primary text-sm font-bold">{{ destination_location.city }},
                                                {{ destination_location.state }}, {{ destination_location.country }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="mb-3 mt-3 flex flex-row justify-end h-max">
                                        <Link :href="route('shipment.destination', shipment.id)">
                                            <button
                                                class="shadow p-2 rounded-full bg-background/30 flex gap-x-3 flex-row justify-between items-center">
                                                <span class="text-primary text-xs">Change</span>
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                ...
                <div class="relative flex flex-row overflow-x-auto duration-500 shadow p-5 rounded-xl hover:shadow-xl">
                    <table class="w-full text-sm text-left text-gray-500 sdark:text-gray-400">
                        ...
                        <tr class="bg-white sdark:bg-gray-800 sdark:border-gray-700">
                            <th scope="row"
                                class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap sdark:text-white">
                                Item Category
                            </th>
                            <td class="px-6 py-2">
                                {{ item_category.name }}
